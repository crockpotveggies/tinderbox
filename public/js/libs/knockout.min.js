...
ko.bindingHandlers.slider = {
  init: function (element, valueAccessor, allBindingsAccessor) {
    var options = allBindingsAccessor().sliderOptions || {start: [20,30], range: {min:18, max:70}, step: 1};
    $(element).noUiSlider(options);

    ko.utils.registerEventHandler(element, "set", function (event, val) {
      var observable = valueAccessor();
      if(typeof val=="string") { observable(Number(val)) }
      else { observable(val); }
    });
    ko.utils.domNodeDisposal.addDisposeCallback(element, function () {
      $(element).remove();
    });
    /*ko.utils.registerEventHandler(element, "slide", function (event, val) {
      var observable = valueAccessor();
      observable(val);
    });*/
  },
  update: function (element, valueAccessor) {
    try {
      ...
      console.warn("There was a problem updating the Knockout slider.")
      console.log(e)
    }
  }
};
