...
import models.bot.Throttler._
import models.bot.BotThrottle._
...
object Throttler {
  ...
  /**
   * Set a monitor of a throttler.
   *
   * You may change a throttler's monitor at any time.
   *
   * @param monitor is an Actor that will receive updates about the size of a data queue if set. This can be used for processing
   ...
   *
   * @param length an Integer stating how many elements are in the queue
   */
  case class QueueState(length: Int)

  /**
   * Skip the queue and directly to the target.
   *
   * Useful for when you need to expedite processing of certain objects, but want to maintain the functionality of the queue. If
   * a throttle is idle, then the message will be enqueued.
   *
   * @param message the message to be sent to the target.
   */
  case class SkipQueue(message: Any)
 ...
 * INTERNAL API
 */
private[bot] object BotThrottle {
  case object Tick
  case object Check

  // States of the FSM: A `TimerBasedThrottler` is in state `Active` if the timer is running.
  sealed trait State
 ...
 * @see [[akka.contrib.throttle.Throttler]]
 */
class BotThrottle(var rate: Rate, var monitor: Option[ActorRef]) extends Actor with FSM[State, Data] {
  ...
  when(Idle) {
    ...
    // Send state back to monitor
    case Event(Check, d) =>
      reportMonitoring(d.queue.length)
      stay using d.copy(callsLeftInThisPeriod = rate.numberOfCalls)

    // Skip the queue
    case Event(SkipQueue(msg), d @ Data(None, _, queue)) =>
      stay using d.copy(queue = queue.enqueue(Message(msg, context.sender)))
    case Event(SkipQueue(msg), d @ Data(Some(_), _, Seq())) =>
  ...
  when(Active) {
    ...
    // Skip the queue and send directly to target
    case Event(SkipQueue(msg), d @ Data(Some(target), _, _)) =>
      target ! msg
      stay using d
  ...
  private def startMonitoring = {
    setTimer("monitorQueue", Check, Duration(1, TimeUnit.MINUTES), true)
    log.info("[throttle] monitoring reports will be sent to %s" format this.monitor.toString)
  }
  private def reportMonitoring(length: Int) = this.monitor match {
    case Some(m) => m ! QueueState(length)
    case None =>
  }
